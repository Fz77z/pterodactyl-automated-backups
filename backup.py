...
logging.basicConfig(
    filename=log_file, level=logging.INFO, format=log_format, filemode="a"
)  # 'a' stands for append
    ...
    for server_id in server_ids:
        try:
            ...
            if backup.status_code == 200:
                logger.info(
                    f"Backup succeeded for server {server_id}. Status code: {backup.status_code}"
                )
                time.sleep(2)
            else:
                failed_servers.append(server_id)
                logger.error(
                    f"Backup failed for server {server_id}. Error code: {backup.status_code}"
                )
                time.sleep(30)
        except requests.exceptions.RequestException as e:
            logger.error(
                f"An error occurred while making the backup request for server {server_id}: {str(e)}"
            )
            failed_servers.append(server_id)
            time.sleep(30)
